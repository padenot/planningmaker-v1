...
var $h_max;				 //maximal height for multicells
...
function Cell($w,$h=0,$txt='',$border=0,$ln=0,$align='',$fill=0,$link='')
{
	...
	$underlined_txt=explode("§",$txt);
	$underlined_txt=$underlined_txt[1];
	$bold_txt=explode("|",$txt);
	$bold_txt=$bold_txt[1];
	...
	{		
		...
		if ($bold_txt!="") $this->SetFont('','B','');
			
		$s.=sprintf('BT %.2f %.2f Td (%s) Tj ET',($this->x+$dx)*$k,($this->h-($this->y+.5*$h+.3*$this->FontSize))*$k,$txt);

		if ($underlined_txt!="") $s.=' '.$this->_dounderline($this->x+$dx,$this->y+.5*$h+.3*$this->FontSize,$underlined_txt);
...
function MultiCell($w,$h,$txt,$border=0,$ln=0,$align='J',$fill=0)
{
    $begin_y=$this->y;
//	$h_max=0;
...
/*
	...
	else $this->Cell($w,$this->h_max+$h-$h_max,substr($s,$j,$i),$b,2,$align,$fill);
*/


//pour décaler sur la colonne suivante
	if($ln>0)
	{
		//Go to next line
		$this->y+=$h;
...
//Tableau coloré
function FancyTable($header,$data,$border=1)
{
...
//		$this->h_max=0;
		$fill=0;
		$this->y=$y_pos_0;
		if ($row[0]=="Pause") $fill=1;
		$this->MultiCell($w[0],4,$row[0],$border,0,'L',$fill);
		$y_pos_0=$this->y;
		$fill=0;
		$this->y=$y_pos_1;
		if ($row[1]=="Pause") $fill=1;
		$this->MultiCell($w[1],4,$row[1],$border,0,'L',$fill);
		$y_pos_1=$this->y;
		$fill=0;
		$this->y=$y_pos_2;
		if ($row[2]=="Pause") $fill=1;
		$this->MultiCell($w[2],4,$row[2],$border,0,'L',$fill);
		$y_pos_2=$this->y;
		$fill=0;
		$this->y=$y_pos_3;
		if ($row[3]=="Pause") $fill=1;
		$this->MultiCell($w[3],4,$row[3],$border,0,'L',$fill);
		$y_pos_3=$this->y;
		$this->Ln();
	}
	$this->h_max=max($y_pos_0,$y_pos_1,$y_pos_2,$y_pos_3);
